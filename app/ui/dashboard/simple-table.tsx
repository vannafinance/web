      ...
      <table className="w-full max-w-full table-auto border-collapse">
        ...
        <tbody>
          {Object.entries(data).map(([asset, values], rowIndex) => (
            <tr key={rowIndex}>
              <td className="py-1 px-2 text-baseBlack text-xs font-semibold border-r border-neutral-300">
                {asset}
              </td>
              {headers.slice(1).map((header, colIndex) => (
                <td
                  key={`${asset}-${header}`}
                  className={`py-1 px-2 text-baseBlack text-xs font-normal text-center ${
